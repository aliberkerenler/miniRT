...
t_ray	get_camera_ray(t_camera *cam, double u, double v)
{
	...
	horizontal = vec3_mul(right, viewport_width);
	vertical = vec3_mul(up, -viewport_height);

	upper_left = cam->position;
	upper_left = vec3_add(upper_left, vec3_mul(forward, focal_length));
	upper_left = vec3_sub(upper_left, vec3_div(horizontal, 2.0));
	upper_left = vec3_sub(upper_left, vec3_div(vertical, 2.0));
